...
contract("", async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        instance2 = await NFTStaking.new({ from: deployer });
    });

    describe("CUTT token", () => {
        it("mint cutt token", async () => {
            await instance1.setNFTStakingAddress(instance2.address);

            await instance1.setLiquidityAddress(accounts[1]);
            await instance1.setCuttiesAddress(accounts[1]);
            await instance1.setTreasuryAddress(accounts[1]);
            await instance1.setV3StakingAddress(accounts[1]);
            await instance1.setSmartFarmingAddress(accounts[1]);
            ...
            const tickLower = 161160
            const tickUpper = 162240
