...
contract V3Staking is ReentrancyGuard, Ownable {
    ...
    function mintV3StakingToken() public onlyOwner {
        require(_cuttToken != address(0));
        ICUTTToken(_cuttToken).mintV3StakingToken();
    }
        ...
        returns (uint256)
    {
        ...
        return
            calcMultiplier(staker[index].lockWeek).mul(staker[index].liquidity);
    }
    ...
    function stake(uint256 tokenId, uint128 lockWeek) public nonReentrant {
        ...
        _totalStakedAmount = _totalStakedAmount.add(
            liquidity.mul(calcMultiplier(lockWeek))
        );
