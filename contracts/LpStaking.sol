...
contract LpStaking is ReentrancyGuard, Ownable {
    ...
    function setPairAddressAddress(address pairAddress) public onlyOwner {
        _cuttEthPair = pairAddress;
    }

    function mintSmartFarmingToken() public onlyOwner {
        require(_cuttToken != address(0));
        ICUTTToken(_cuttToken).mintSmartFarmingToken();
    }
    ...
    function stake(uint256 amount, uint128 lockWeek) public nonReentrant {
        ...
        IERC20(_cuttEthPair).transferFrom(
            _msgSender(),
            address(this),
            amount
        );
    ...
    function withdraw(uint256 index, uint256 amount) public nonReentrant {
        ...
        IERC20(_cuttEthPair).transfer(_msgSender(), amount);
