...
contract NFTStaking is ReentrancyGuard, Ownable {
    ...
    function mintNFTStakingToken() public onlyOwner {
        require(_cuttToken != address(0));
        ICUTTToken(_cuttToken).mintNFTStakingToken();
    }
    ...
    function stake(uint256 tokenId, uint128 lockWeek) public nonReentrant {
        ...
        address allowanceAddress = IERC721(_cutties).getApproved(tokenId);
        require(
            allowanceAddress == address(this),
            "Stake: Token is not approved."
        );
        StakeInfo[] storage staker = _stakers[_msgSender()];
        IERC721(_cutties).transferFrom(_msgSender(), address(this), tokenId);
        staker.push(
            StakeInfo({
                id: tokenId,
    ...
    function withdraw(uint256 index) public nonReentrant {
        ...
        IERC721(_cutties).approve(_msgSender(), tokenId);
        IERC721(_cutties).transferFrom(address(this), _msgSender(), tokenId);
        ...
        ICUTTToken(_cuttToken).burn(pending);
        ICUTTToken(_cuttToken).transfer(_msgSender(), available);
